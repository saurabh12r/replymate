...
class ExpiredUsersScreen extends ConsumerStatefulWidget {
  const ExpiredUsersScreen({super.key});

  @override
  ConsumerState<ExpiredUsersScreen> createState() => _ExpiredUsersScreenState();
}

class _ExpiredUsersScreenState extends ConsumerState<ExpiredUsersScreen> {
  String _search = '';
  int _page = 0;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          PageHeader(
            title: 'Expired Users',
            subtitle: 'Users whose subscription has ended',
                ...
                children: [
                  ...
                  if (pageUsers.isEmpty)
                    const EmptyState(
                      ...
                      message: 'All users have active subscriptions.',
                    )
                  else
                    AppCard(
                      padding: EdgeInsets.zero,
                      child: LayoutBuilder(builder: (ctx, constraints) {
                        final tableWidth = constraints.maxWidth > 800 ? constraints.maxWidth.toDouble() : 800.0;
                        return SingleChildScrollView(
...
class _ExpiredUserTile extends StatefulWidget {
  final ReplymetUser user;
  const _ExpiredUserTile({required this.user});

  @override
  State<_ExpiredUserTile> createState() => _ExpiredUserTileState();
}

class _ExpiredUserTileState extends State<_ExpiredUserTile> {
  bool _hovered = false;
