...
class SettingsController extends GetxController {
  ...
  final RxBool isIgnoringBatteryOptimizations = true.obs;

  final RxBool autoReplyEnabled = true.obs;
  final RxBool throttleEnabled = true.obs;
  final RxInt throttleDuration = 1.obs;

  static const List<int> throttleDurationOptions = [1, 3, 6, 12, 24];

  String get throttleDurationText {
    final hours = throttleDuration.value;
  ...
  void onInit() {
    ...
    ever<int>(throttleDuration, (value) async {
      if (_hydrating) return;
      await _autoReplyPreferences.setThrottleDuration(value);
      await _autoReplyBridge.setThrottleDuration(value);
    });
    refreshPermissionStatus();
  }

  Future<void> refreshPermissionStatus() async {
    permPhoneCallLogsGranted.value = await _permissionService.isGranted(
      AppPermissionType.callLogs,
    );
    permPostNotificationsGranted.value = await _permissionService.isGranted(
      AppPermissionType.notifications,
    );
    if (Platform.isAndroid) {
      isIgnoringBatteryOptimizations.value =
          await _autoReplyBridge.isIgnoringBatteryOptimizations();
    } else {
      isIgnoringBatteryOptimizations.value = true;
    }
  }

  Future<void> requestIgnoreBatteryOptimizations() async {
    if (Platform.isAndroid) {
      await _autoReplyBridge.requestIgnoreBatteryOptimizations();
  ...
  Future<void> _loadPersistedRules() async {
    _hydrating = true;
    try {
      autoReplyEnabled.value = await _autoReplyPreferences
          .getAutoReplyEnabled();
      throttleEnabled.value = await _autoReplyPreferences.getThrottleEnabled();
      throttleDuration.value = await _autoReplyPreferences
          .getThrottleDuration();
    } finally {
      _hydrating = false;
    }
  ...
  Future<void> saveSettings() async {
    ...
    await _autoReplyPreferences.setThrottleDuration(throttleDuration.value);
    ...
    await _autoReplyBridge.setThrottleDuration(throttleDuration.value);
  ...
  Future<void> showThrottleDurationPicker() async {
    final result = await Get.dialog<int>(
      _ThrottleDurationDialog(
        currentDuration: throttleDuration.value,
        onSelect: (duration) {
          throttleDuration.value = duration;
        },
      ),
  ...
  Future<void> recheckPermissions() async {
    ...
    await refreshPermissionStatus();
    final ok =
        permPhoneCallLogsGranted.value && permPostNotificationsGranted.value;
    Get.snackbar(
      'Permissions',
      ok
      ...
      snackPosition: SnackPosition.BOTTOM,
      backgroundColor: ok ? const Color(0xFF006A6A) : const Color(0xFF24389C),
      colorText: Colors.white,
      borderRadius: 14,
      margin: const EdgeInsets.all(16),
    );
...
class _ThrottleDurationDialog extends StatefulWidget {
  final int currentDuration;
  final Function(int) onSelect;

  const _ThrottleDurationDialog({
    required this.currentDuration,
    required this.onSelect,
  });

  @override
  State<_ThrottleDurationDialog> createState() =>
      _ThrottleDurationDialogState();
}

class _ThrottleDurationDialogState extends State<_ThrottleDurationDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    return AlertDialog(
      ...
      content: Column(
        ...
        children: [
          Text(
            'How long should we wait before sending another auto-reply to the same contact?',
          ...
          ...SettingsController.throttleDurationOptions.map((hours) {
      ...
      actions: [
        ...
        FilledButton(
          onPressed: () {
            ...
            Get.back(result: _selectedDuration);
          },
