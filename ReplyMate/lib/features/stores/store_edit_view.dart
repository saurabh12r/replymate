...
class StoreEditView extends StatefulWidget {
  const StoreEditView({super.key, required this.store, this.isNew = false});
...
class _StoreEditViewState extends State<StoreEditView> {
  ...
  void _applyGroupMessage() {
    ...
    if (text.isEmpty || _groupKeys.isEmpty) {
      Get.snackbar(
        'Select types',
        'Choose at least one type and enter a message.',
      );
      return;
    }
  ...
  Future<void> _save({bool pop = true}) async {
    ...
    if (sid != null && !infos.any((e) => e['subscriptionId'] == sid)) {
      sid = null;
    }
    ...
    if (widget.isNew) {
      ordered = [..._c.stores.map((s) => byId[s.id] ?? s), updated];
    } else {
