...
class ContactFilterController extends GetxController {
  ContactFilterController({ContactFilterPreferences? preferences})
  ...
  int get selectedContactsCount {
    if (mode.value == ContactFilterMode.all) return 0;
    return _selectionSetForMode(mode.value).length;
  }

  List<ContactPhoneRow> get selectedContacts {
    if (mode.value == ContactFilterMode.all) return [];
    final digitsSet = _selectionSetForMode(mode.value);
    return _allRowsCache.where((r) => digitsSet.contains(r.digitsKey)).toList();
  }

  List<ContactPhoneRow> get allContacts => _allRowsCache;
  ...
  Future<void> _loadFromDisk() async {
    ...
    excludeSelectedDigits.value = await _preferences
        .getExcludeSelectedPhoneDigits();
  ...
  void _throttledLoadMore() {
    ...
    if (_lastLoadMoreAt != null &&
        now.difference(_lastLoadMoreAt!) < const Duration(milliseconds: 200)) {
      return;
    }
    _lastLoadMoreAt = now;
    loadMoreVisible();
  ...
  Future<void> loadContacts({bool forceRefresh = false}) async {
    ...
    try {
      ...
      final rows = sortedMaps
          .map(ContactPhoneRow.fromMap)
  ...
  void _recomputeFilteredCache() {
    ...
    if (qTrim.isEmpty) {
      ...
      final digitsSet = _selectionSetForMode(mode.value);
      final selected = base
          .where((r) => digitsSet.contains(r.digitsKey))
          .toList(growable: false);
      final others = base
          .where((r) => !digitsSet.contains(r.digitsKey))
          .toList(growable: false);
  ...
  Future<void> syncPolicyFromDisk() async {
    try {
      mode.value = await _preferences.getMode();
      onlySelectedDigits.value = await _preferences
          .getOnlySelectedPhoneDigits();
      excludeSelectedDigits.value = await _preferences
          .getExcludeSelectedPhoneDigits();
      ContactFilterSmsPolicy.instance.apply(
        mode: mode.value,
  ...
  Future<void> save(BuildContext context) async {
    ...
    if (mode.value == ContactFilterMode.onlySelected &&
        activeSelected.isEmpty) {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('Contact filter saved')));
    } finally {
      saving.value = false;
    }
  }
