...
class DashboardController extends GetxController {
  ...
  Future<void> _loadAutoReplyState() async {
    try {
      ...
      // Compute the effective enabled state.
      // If admin says blocked/expired, always force OFF regardless of local pref.
      final enabled = enabledPref && allowed;
      autoReplyEnabled.value = enabled;
      status.value = enabled ? 'Operational' : 'Paused';

      // Only push to native when we need to DEMOTE (turn OFF) due to admin rules.
      // NEVER push true to native here — the native layer is the authoritative
      // source for the enabled state when the app is closed. Pushing true from
      // the Dart layer on startup could re-enable an engine the user explicitly
      // turned off (or that was blocked by a subscription event while app was closed).
      if (!enabled && enabledPref) {
        // Admin says blocked — force both layers to OFF.
        await _autoReplyPreferences.setAutoReplyEnabled(false);
        await _autoReplyBridge.setAutoReplyEnabled(false);
      }
  ...
  Future<bool> _isAutoReplyAllowedInFirebase() async {
    ...
    String? docId = currentUser.phoneNumber;
    if (docId == null || docId.isEmpty) {
      docId = await _userRepository.getUserPhoneByUid(currentUser.uid);
    }
    if (docId == null || docId.isEmpty) return false;

    final data = await _userRepository.getUserByPhone(docId);
