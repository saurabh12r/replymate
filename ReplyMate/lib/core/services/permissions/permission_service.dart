...
enum AppPermissionType { sms, callLogs, contacts, notifications }
...
class PermissionService {
  ...
  Future<bool> _isCallLogsGranted() async {
    ...
    try {
      final isGranted = await platform.invokeMethod<bool>('checkCallLogPermission');
      final phoneState = await Permission.phone.isGranted;
      return (isGranted ?? false) && phoneState;
    } catch (e) {
      return false;
    }
  }
  ...
  Future<PermissionStatus> _requestCallLogPermissions() async {
    ...
    const platform = MethodChannel('replymate/call_log');
    try {
      final callLogGranted = await platform.invokeMethod<bool>('requestCallLogPermission');
      if (phoneStatus.isGranted && (callLogGranted ?? false)) {
