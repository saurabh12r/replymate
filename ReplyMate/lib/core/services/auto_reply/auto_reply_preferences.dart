...
class AutoReplyPreferences {
  ...
  static const String _keyThrottleDuration = 'throttle_duration';
  ...
  Future<bool> getAutoReplyEnabled() async {
    ...
    // Default is FALSE — matches the native layer's fail-safe default.
    // If the key is missing the engine should be OFF until the user enables it.
    return prefs.getBool(_keyAutoReplyEnabled) ?? false;
  }
  ...
  Future<int> getThrottleDuration() async {
    final prefs = await SharedPreferences.getInstance();
    return prefs.getInt(_keyThrottleDuration) ?? 1;
  }

  Future<void> setThrottleDuration(int hours) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt(_keyThrottleDuration, hours);
  }
